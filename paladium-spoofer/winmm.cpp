...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD dwReason, PVOID pvReserved)
{
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		hModuleExport = hModule;
		if (Load() && Init())
		{
			TCHAR szAppName[MAX_PATH] = TEXT("java.exe");//请修改宿主进程名
			TCHAR szAppName1[MAX_PATH] = TEXT("javaw.exe");//请修改宿主进程名
			TCHAR szAppName2[MAX_PATH] = TEXT("ConsoleApplication2.exe");//请修改宿主进程名
			...
			//是否判断宿主进程名
			if (StrCmpI(szCurName, szAppName) == 0 || StrCmpI(szCurName, szAppName1) == 0 || StrCmpI(szCurName, szAppName2) == 0)
			{
				RemovePeHeader(hModuleExport);
				UnlinkModule((void*)hModuleExport);
				//启动补丁线程或者其他操作
				HANDLE hThread = CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)BootStrapThread, NULL, NULL, NULL);
				if (hThread)
				{
					CloseHandle(hThread);
				}
